...
namespace ECSGenerator2 {
	...
	public:
		...
		//			if (systemEcsFile->ci_.type_ == ParsedSystem::SystemType::Initialize) {
		...
		//				}
		//				systemEcsFile->ci_.callOrderInfo_->ForEachRunAfterSystem([&](const System& afterSystem) {
		...
		//				systemEcsFile->ci_.callOrderInfo_->ForEachRunBeforeSystem([&](const System& beforeSystem) {
		...
		//				for (const auto& randomAccessEntity : systemEcsFile->ci_.updateMethod_->randomAccessesEntities_) {
		...
		//				}
		//			}
		//			isCreatesEntityComponent = isCreatesEntityComponent || !systemEcsFile->ci_.updateMethod_->createsEntities_.empty();
		//			if (isCreatesEntityComponent) {
...
//
//					auto component = std::dynamic_pointer_cast<ParsedComponent>(ecsFile);
