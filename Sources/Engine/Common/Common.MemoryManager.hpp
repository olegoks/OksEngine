...
namespace Common::Memory {
	...
	public:
		MemoryManager(Common::Size memoryPoolSize) noexcept :
	...
	class TagedMemoryManager : public MemoryManager {
	public:
		...
		TagedMemoryManager(Common::Size memoryPoolSize) noexcept :
			MemoryManager{ memoryPoolSize } { }

		template<class Type, class ... Args>
		SharedPointer<Type> MakeShared(Tag&& tag, Args&& ... args) noexcept {

			return MemoryManager::MakeShared<Type>(std::forward<Args&&>(args)...);
		}
		...
		Type* Allocate(Tag&& tag, Common::Size size) noexcept {
			return MemoryManager::Allocate<Type>(size);
		}
	...
	[[nodiscard]]
	TagedMemoryManager::Tag operator""_tag(const char* tagString) noexcept {
		return TagedMemoryManager::Tag{ tagString };
	}


	TagedMemoryManager memory{ 64_MB };
