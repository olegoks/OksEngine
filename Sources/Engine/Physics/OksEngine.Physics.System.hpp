...
namespace OksEngine {
	...
	class CreateRigidBodyCustomMeshShape : public ECSSystem {
	public:

		CreateRigidBodyCustomMeshShape(Context& context) : ECSSystem{ context } {
		...
		virtual void Update(ECS::World* world, ECS::Entity::Id entityId, ECS::Entity::Id secondEntityId) override {
			...
			Mesh* mesh = world->GetComponent<Mesh>(entityId);
			DynamicRigidBodyCustomMeshShape* dynamicRigidBody = world->GetComponent<DynamicRigidBodyCustomMeshShape>(entityId);
			
			ModelStorage::Model& model = GetContext().GetModelStorage()->GetModel(mesh->modelId_);
